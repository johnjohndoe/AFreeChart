...
public class FlowArrangement implements Arrangement, Serializable {
     ...
     * container, subject to the given constraint. The <code>Canvas</code>
    ...
    public Size2D arrange(BlockContainer container, Canvas canvas,
        ...
        if (w == LengthConstraintType.NONE) {
            if (h == LengthConstraintType.NONE) {
                return arrangeNN(container, canvas);
            } else if (h == LengthConstraintType.FIXED) {
                return arrangeNF(container, canvas, constraint);
            } else if (h == LengthConstraintType.RANGE) {
                throw new RuntimeException("Not implemented.");
            }
        } else if (w == LengthConstraintType.FIXED) {
            if (h == LengthConstraintType.NONE) {
                return arrangeFN(container, canvas, constraint);
            } else if (h == LengthConstraintType.FIXED) {
                return arrangeFF(container, canvas, constraint);
            } else if (h == LengthConstraintType.RANGE) {
                return arrangeFR(container, canvas, constraint);
            }
        } else if (w == LengthConstraintType.RANGE) {
            if (h == LengthConstraintType.NONE) {
                return arrangeRN(container, canvas, constraint);
            } else if (h == LengthConstraintType.FIXED) {
                return arrangeRF(container, canvas, constraint);
            } else if (h == LengthConstraintType.RANGE) {
                return arrangeRR(container, canvas, constraint);
            }
        }
     ...
     * @return The size.
     */
    protected Size2D arrangeFN(BlockContainer container, Canvas canvas,
            RectangleConstraint constraint) {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            Size2D size = block.arrange(canvas, RectangleConstraint.NONE);
            if (x + size.width <= width) {
                itemsInRow.add(block);
                block.setBounds(new RectShape(x, y, size.width,
                        size.height));
                ...
                if (itemsInRow.isEmpty()) {
                    ...
                    block.setBounds(new RectShape(x, y, Math.min(
    ...
    protected Size2D arrangeFR(BlockContainer container, Canvas canvas,
        ...
        if (constraint.getHeightRange().contains(s.height)) {
            ...
            return arrangeFF(container, canvas, c);
        }
    }
     ...
     */
    protected Size2D arrangeRR(BlockContainer container, Canvas canvas,
            RectangleConstraint constraint) {

        // first arrange without constraints, and see if this fits within
        // the required ranges...
        Size2D s1 = arrangeNN(container, canvas);
        if (constraint.getWidthRange().contains(s1.width)) {
            ...
            return arrangeFR(container, canvas, c);
        }
    }
     ...
     * @return The size following the arrangement.
     */
    protected Size2D arrangeRF(BlockContainer container, Canvas canvas,
            RectangleConstraint constraint) {

        Size2D s = arrangeNF(container, canvas, constraint);
     ...
     * @return The size following the arrangement.
     */
    protected Size2D arrangeRN(BlockContainer container, Canvas canvas,
            RectangleConstraint constraint) {
        ...
        if (constraint.getWidthRange().contains(s1.width)) {
            ...
            RectangleConstraint c = constraint.toFixedWidth(constraint
                    .getWidthRange().getUpperBound());
            return arrangeFN(container, canvas, c);
        }
    }
    ...
    protected Size2D arrangeNN(BlockContainer container, Canvas canvas) {
        ...
        if (blockCount > 0) {
            ...
            for (int i = 0; i < blocks.size(); i++) {
                Block block = (Block) blocks.get(i);
                sizes[i] = block.arrange(canvas, RectangleConstraint.NONE);
                width = width + sizes[i].getWidth();
                maxHeight = Math.max(sizes[i].height, maxHeight);
                block.setBounds(new RectShape(x, 0.0, sizes[i].width,
                        sizes[i].height));
     ...
     * @return The size after the arrangement.
     */
    protected Size2D arrangeNF(BlockContainer container, Canvas canvas,
            RectangleConstraint constraint) {
