...
import org.afree.graphics.geom.Area;
...
public class PiePlot3D extends PiePlot implements Serializable {
     ...
     * {@link org.AFreeChartChart.chart.AFreeChart} class, you don't normally need
     * to call it yourself.
     *
     ...
     */
    public void draw(Canvas canvas, RectShape plotArea, PointF anchor,
        ...
        RectShape originalPlotArea = (RectShape) plotArea.clone();
        if (info != null) {
            info.setPlotArea(plotArea);
            info.setDataArea(plotArea);
        }

        drawBackground(canvas, plotArea);
        
        RectShape savedClip = new RectShape(canvas.getClipBounds());
        canvas.clipRect(plotArea.getMinX(), plotArea.getMinY(), plotArea.getMaxX(), plotArea.getMaxY());
        ...
        if (DEBUG_DRAW_INTERIOR) {
            ...
            Paint p = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, DEFAULT_LABEL_BACKGROUND_PAINT_TYPE);
            p.setColor(Color.LTGRAY);
            
            new RectShape(igx1, igy1, igx2 - igx1,
                    igy2 - igy1).draw(canvas, p);
        }
        ...
        // the labels
        RectShape linkAreaXX = new RectShape(linkX, linkY, linkW,
                linkH * (1 - this.depthFactor));
        state.setLinkArea(linkAreaXX);

        if (DEBUG_DRAW_LINK_AREA) {
            Paint p = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, DEFAULT_LABEL_OUTLINE_PAINT_TYPE);
            p.setColor(Color.BLUE);
            linkAreaXX.draw(canvas, p);
            p.setColor(Color.YELLOW);
            new OvalShape(linkAreaXX.getX(), linkAreaXX.getY(),
                    linkAreaXX.getWidth(), linkAreaXX.getHeight()).draw(canvas, p);
        }
        ...
        RectShape explodeArea = new RectShape(linkX + hh / 2.0,
        ...
        RectShape pieArea = new RectShape(explodeArea.getX()
        ...
        RectShape linkArea = new RectShape(linkX, linkY, linkW,
                linkH - depth);
        ...
        if (dataset.getKeys().size() > plotArea.getWidth()) {
            ...
            //FontMetrics fm = canvas.getFontMetrics(sfont);
            //int stringWidth = fm.stringWidth(text);
            // need FontMetrics class for text centering
            int stringWidth = 0;
            Paint p = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, DEFAULT_LABEL_PAINT_TYPE, 
                    new Font(Typeface.DEFAULT_BOLD, 10, stringWidth)
                    );

            p.setTypeface(Typeface.create(Typeface.DEFAULT_BOLD, 0));
            p.setTextSize(10);
            canvas.drawText(text, (int) (plotArea.getX() + (plotArea.getWidth()
                    - stringWidth) / 2), (int) (plotArea.getY()
                            + (plotArea.getHeight() / 2)), p);
        ...
        if (isCircular()) {
            ...
            plotArea = new RectShape(plotArea.getCenterX() - min,
        ...
        while (iterator.hasNext()) {
            ...
            if (Math.abs(angle2 - angle1) > getMinimumArcAngleToDraw()) {
                arcList.add(new ArcShape(arcX, arcY + depth,
                        pieArea.getWidth(), pieArea.getHeight() - depth,
                        angle1, angle2 - angle1, true));
            }
            else {
                arcList.add(null);
            }
        ...
        Shape oldClip = new RectShape(canvas.getClipBounds());

        OvalShape top = new OvalShape(pieArea.getX(), pieArea.getY(),
                pieArea.getWidth(), pieArea.getHeight() - depth);

        OvalShape bottom = new OvalShape(pieArea.getX(), pieArea.getY()
                + depth, pieArea.getWidth(), pieArea.getHeight() - depth);

        RectShape lower = new RectShape(top.getX(),
                top.getCenterY(), pieArea.getWidth(), bottom.getMaxY()
                - top.getCenterY());

        RectShape upper = new RectShape(pieArea.getX(), top.getY(),
        ...
        int[] xs;
        int[] ys;
        arc = new ArcShape(arcX, arcY + depth, pieArea.getWidth(),
                pieArea.getHeight() - depth, 0, 360, true);

        int categoryCount = arcList.size();
        for (int categoryIndex = 0; categoryIndex < categoryCount;
                 categoryIndex++) {
            arc = (ArcShape) arcList.get(categoryIndex);
            if (arc == null) {
                continue;
            }
            Comparable key = getSectionKey(categoryIndex);
            
            Paint serise = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, lookupSectionPaintType(key));
            Paint outline = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, lookupSectionOutlinePaintType(key), lookupSectionOutlineStroke(key), lookupSectionOutlineEffect(key));
            
            arc.fill(canvas, serise);
            arc.draw(canvas, outline);
            
            PointF p1 = arc.getStartPoint();

            // draw the height
            xs = new int[] {(int) arc.getCenterX(), (int) arc.getCenterX(),
                    (int) p1.x, (int) p1.x};
            ys = new int[] {(int) arc.getCenterY(), (int) arc.getCenterY()
                    - depth, (int) p1.y - depth, (int) p1.y};
            Polygon polygon = new Polygon(xs, ys, 4);
            serise.setColor(Color.LTGRAY);
            polygon.fill(canvas, serise);
            polygon.draw(canvas, outline);
        }
        
        Paint paint = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, new SolidColor(Color.LTGRAY));
        back.fill(canvas, paint);
        front.fill(canvas, paint);

        // cycle through once drawing only the sides at the back...
        int cat = 0;
        iterator = arcList.iterator();
        while (iterator.hasNext()) {
            ArcShape segment = (ArcShape) iterator.next();
            if (segment != null) {
                Comparable key = getSectionKey(cat);
                drawSide(canvas, pieArea, segment, front, back, lookupSectionPaintType(key),
                        lookupSectionOutlinePaintType(key), lookupSectionOutlineStroke(key), lookupSectionOutlineEffect(key), false, true);
            }
            cat++;
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (segment != null) {
                Comparable key = getSectionKey(cat);
                drawSide(canvas, pieArea, segment, front, back, lookupSectionPaintType(key),
                        lookupSectionOutlinePaintType(key), lookupSectionOutlineStroke(key), lookupSectionOutlineEffect(key), true, false);
            }
            cat++;
        }
        ...
        for (int sectionIndex = 0; sectionIndex < categoryCount;
                 sectionIndex++) {
            arc = (ArcShape) arcList.get(sectionIndex);
            if (arc == null) {
                continue;
            }
            upperArc = new ArcShape(arcX, arcY, pieArea.getWidth(),
                    pieArea.getHeight() - depth, arc.getAngleStart(),
                    arc.getAngleExtent(), true);

            Comparable currentKey = (Comparable) sectionKeys.get(sectionIndex);
            Paint serise = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, lookupSectionPaintType(currentKey));
            Paint outline = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, lookupSectionOutlinePaintType(currentKey), lookupSectionOutlineStroke(currentKey), lookupSectionOutlineEffect(currentKey));
            
            upperArc.fill(canvas, serise);
            upperArc.draw(canvas, outline);
        ...
        RectShape adjustedPlotArea = new RectShape(
                originalPlotArea.getX(), originalPlotArea.getY(),
                originalPlotArea.getWidth(), originalPlotArea.getHeight()
                - depth);
        if (getSimpleLabels()) {
            drawSimpleLabels(canvas, keys, totalValue, adjustedPlotArea,
                    linkArea, state);
        }
        else {
            drawLabels(canvas, keys, totalValue, adjustedPlotArea, linkArea,
                    state);
        }

        canvas.clipRect(savedClip.getMinX(), savedClip.getMinY(), savedClip.getMaxX(), savedClip.getMaxY());
        drawOutline(canvas, originalPlotArea);
     ...
     * @param paintType  the color.
     * @param outlinePaintType  the outline paint.
     * @param outlineStroke  the outline stroke.
     * @param drawFront  draw the front?
     * @param drawBack  draw the back?
     */
    protected void drawSide(Canvas canvas,
                            RectShape plotArea,
                            ArcShape arc,
                            Area front,
                            Area back,
                            PaintType paintType,
                            PaintType outlinePaintType,
                            float outlineStroke,
                            PathEffect outlineEffect,
                            boolean drawFront,
                            boolean drawBack) {
            ...
            if (isAngleAtFront(start)) {  // start at front
                ...
                if (!isAngleAtBack(end)) {
                    ...
                    if (extent > -180.0) {  // the segment is entirely at the
                                            // front of the chart
                        if (drawFront) {
                            Area side = new Area(new RectShape(
                                    arc.getEndPoint().x, plotArea.getY(),
                                    arc.getStartPoint().x
                                    - arc.getEndPoint().x,
                                    plotArea.getHeight()));
                            side.intersect(front);
                            side.fill(canvas, paint);
                            side.draw(canvas, outlinePaint);
                        }
                    }
                    else {  // the segment starts at the front, and wraps all
                            // the way around
                            // the back and finishes at the front again
                        Area side1 = new Area(new RectShape(
                                plotArea.getX(), plotArea.getY(),
                                arc.getStartPoint().x - plotArea.getX(),
                                plotArea.getHeight()));
                        side1.intersect(front);

                        Area side2 = new Area(new RectShape(
                                arc.getEndPoint().x, plotArea.getY(),
                                plotArea.getMaxX() - arc.getEndPoint().x,
                                plotArea.getHeight()));

                        side2.intersect(front);
                        if (drawFront) {
                            side1.fill(canvas, paint);
                            side2.fill(canvas, paint);
                        }
                        ...
                        if (drawFront) {
                            side1.draw(canvas, outlinePaint);
                            side2.draw(canvas, outlinePaint);
                        }

                        if (drawBack) {
                            back.draw(canvas, outlinePaint);
                        }
                    ...
                    if (drawBack) {
                        Area side2 = new Area(new RectShape(
                                plotArea.getX(), plotArea.getY(),
                                arc.getEndPoint().x - plotArea.getX(),
                                plotArea.getHeight()));
                ...
                if (!isAngleAtFront(end)) {
                    if (extent > -180.0) {  // whole segment stays at the back
                        if (drawBack) {
                            Area side = new Area(new RectShape(
                                    arc.getStartPoint().x, plotArea.getY(),
                                    arc.getEndPoint().x
                                    - arc.getStartPoint().x,
                                    plotArea.getHeight()));
                            side.intersect(back);
                        ...
                        Area side1 = new Area(new RectShape(
                                arc.getStartPoint().x, plotArea.getY(),
                                plotArea.getMaxX() - arc.getStartPoint().x,
                                plotArea.getHeight()));
                        ...
                        if (drawFront) {
                            front.draw(canvas, outlinePaint);
                        }
            ...
            if (isAngleAtFront(start)) {  // segment starts at the front
                ...
                if (!isAngleAtFront(end)) {
                    if (extent < 180.0) {  // and finishes at back
                        ...
                        Area side1 = new Area(new RectShape(
                                arc.getStartPoint().x, plotArea.getY(),
                                plotArea.getX() - arc.getStartPoint().x,
                                plotArea.getHeight()));
                        side1.intersect(back);
